...
const hashtagsGroupContainer = document.querySelector(
  ".hashtag_groups_container"
);
let hashtagsGroup = document.querySelector(
  ".hashtag_groups_container"
).children;
...
const totalAudienceCalculation = (c) => {
  if (hashtagsArray[c].audiences.length === 0) {
    ...
    let formatedTA = formatTA(
      hashtagsArray[c].audiences.reduce((a, c) => a + c)
    );
...
function sliderNextFn(arrSlide, arrSlide2) {
  if (
    hashtagsGroup[currentLastHg].children.length >= 30 &&
    curSlide === maxSlide
  ) {
    arrSlide.insertAdjacentHTML("beforeend", `<p>Group ${groupNum}</p>`);
    arrSlide2.insertAdjacentHTML(
      "beforeend",
      `<div class='hashtag_group'></div>`
    );
...
form.addEventListener("submit", (e) => {
  e.preventDefault();
  if (hashtagsGroup[curSlide].children.length >= 30) {
    ...
    if (addNewInput.value !== "" && audianceInput.value !== "") {
      hashtagsArray[curSlide] = {
        ...hashtagsArray[curSlide],
        audiences: [
          ...hashtagsArray[curSlide].audiences,
          Number(audianceInput.value),
        ],
        hashtags: [
          ...hashtagsArray[curSlide].hashtags,
          `#${addNewInput.value}`,
        ],
...
addToTableBtn.addEventListener("click", () => {
  let currentData = {
    ...
    savedBy: "User",
    totalAudience: formatTA(
      hashtagsArray[curSlide].audiences.reduce((a, c) => a + c)
    ),
    totalAudienceNumber: hashtagsArray[curSlide].audiences.reduce(
      (a, c) => a + c
    ),
    successRate: "N/A",
  };
  ...
  document
    .querySelector(".hashtagsGroupTable tbody")
    .insertAdjacentHTML("beforeend", tableDataHtml);
});
...
success.addEventListener("click", function () {
  ...
  if (order === "desc") {
    ...
    tableData.sort((a, b) =>
      a.totalAudienceNumber > b.totalAudienceNumber ? 1 : -1
    );
